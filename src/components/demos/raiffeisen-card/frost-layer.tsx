...
/* The frozen state: the card face turns to frosted glass, spreading from the middle out.
 *
 * It's a port of "Spreading Frost" by dos (shadertoy XddcRr), itself built on Shadmar's "Frosted
 * Glass II" (MsySzy), which the Flutter freeze shader also uses. The recipe: sample the image at
 * noise-jittered coordinates, tint it towards ice, and reveal that through a vignette that grows
 * with `progress`, its edge broken up by a low-frequency "ice spread" noise. The noise textures
 * of the original are generated in the shader instead of loaded.
 *
 * A shader can't read the DOM, so the face is first redrawn into a 2D canvas (background, SVGs,
 * text, the spending bar) from an untransformed clone, and that becomes the texture. The frozen
 * region is drawn opaque over the real card and the thaw edge blends into it, so small
 * differences between the snapshot and the DOM are hidden inside the frost.
 *
 * Each pixel is a pure function of `progress`, so whatever drives it decides the direction: a
 * reversal mid-way walks back through the same frames. Without WebGL2 a frosted gradient fades
 * with the same value.
 *
 * `webgl={false}` takes the gradient on purpose: browsers cap live WebGL contexts, so cards that
 * aren't the focus (carousel neighbours) don't each hold one. Every WebGL session gets a fresh
 * canvas: a canvas whose context was lost hands back that dead context forever. Until the shader
 * has its first snapshot, the gradient stands in, so a frozen card never shows bare.
 */
...
type Stop = readonly [color: string, at: number];
...
async function snapshotFace(face: HTMLElement, stops: readonly Stop[], dpr: number) {
...
function createFrost(canvas: HTMLCanvasElement) {
...
export function FrostLayer({
  ...
  /** The face to freeze; the layer must sit inside it. */
  faceRef: React.RefObject<HTMLElement | null>;
  /** Background gradient of the face, drawn into the snapshot as a 135deg linear gradient. */
  stops: readonly Stop[];
  /** Changes whenever the face looks different, so the snapshot is retaken. */
  version: string;
  /** False draws the plain frosted gradient instead of the shader. */
  webgl?: boolean;
}) {
  const hostRef = useRef<HTMLDivElement>(null);
  const [failed, setFailed] = useState(false);
  const [ready, setReady] = useState(false);
  const fallback = failed || !webgl;
  const frost = useRef<ReturnType<typeof createFrost>>(null);
  ...
  useEffect(() => {
    ...
    // Draws on demand only: a progress change or a new snapshot asks for one frame.
    let raf = 0;
    redraw.current = () => {
      if (!raf) raf = requestAnimationFrame(() => ((raf = 0), f.draw(progress.get(), dpr())));
    };
    ...
    resnap.current = () => {
      ...
      document.fonts.ready
        ...
        .then((snap) => {
          if (!snap || mine !== token || frost.current !== f) return;
          f.setFace(snap);
          redraw.current();
          setReady(true);
        })
        .catch((err) => {
          // A tainted snapshot can't become a texture; the plain fallback still freezes.
          console.warn("frost snapshot failed", err);
          setFailed(true);
        });
    };
  ...
  }, [progress, faceRef, webgl]);

  const stopsRef = useRef(stops);
  stopsRef.current = stops;

  // Retake the snapshot when the face changes, and once more after its own transitions settle
  // (colours cross-fade, digits decode) so the frost shows the final state. The first take waits
  // a beat, so a stream of changes (scrubbing the limit) doesn't clone the face on every step.
  useEffect(() => {
    const t1 = setTimeout(() => resnap.current(), 120);
    const t2 = setTimeout(() => resnap.current(), 800);
    return () => (clearTimeout(t1), clearTimeout(t2));
  }, [version, webgl]);

  useMotionValueEvent(progress, "change", () => redraw.current());

  const fallbackOpacity = useTransform(progress, [0, 1], [0, 1]);

  return (
    <>
      <div ref={hostRef} data-frost-skip className="pointer-events-none absolute inset-0" aria-hidden />
